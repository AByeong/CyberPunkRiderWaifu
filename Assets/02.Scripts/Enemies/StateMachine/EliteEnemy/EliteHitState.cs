using DG.Tweening;
using UnityEngine;
using UnityEngine.AI; // NavMeshAgent를 위해 추가 (Owner에 있다고 가정)
using System.Collections;
using System.Collections.Generic;

// EliteBaseState, EliteDownState, EliteIdleState 등은 실제 프로젝트에 맞게 정의되어 있어야 합니다.
// Owner 객체 (예: EliteEnemy 클래스)에는 Animator, NavMeshAgent, EnemyData, Target 등의 속성이 있다고 가정합니다.

public class EliteHitState : EliteBaseState
{
   // 타이머 및 시퀀스
    ...
    [Range(0f, 5f)]
    public float normalVValue = 0.4f; // 예시 값, 실제 머티리얼 밝기에 맞춰 조정
    ...
    private void CacheRenderersAndInitialColors()
    {
        ...
        normalVValue = Owner.OriginalValue;
